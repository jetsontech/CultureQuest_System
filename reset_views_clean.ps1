...
def get_pluto_style_categories():
    return [
        "Featured", "Movies", "Comedy", "Drama", "Crime", "Reality",
        "Documentary", "News", "Sports", "Games", "Kids", "Anime",
        "Entertainment", "Food", "Travel", "Music", "Lifestyle",
        "History", "Science", "Community", "Creators", "Faith",
        "International", "Local"
    ]
...
def categorize_channel(channel):
    ...
    rules = [
        ...
        ("Reality", ["reality"]),
        ("Documentary", ["documentary", "docs", "nature", "history"]),
...
def build_category_rows(channels):
    ...
    for name in category_order:
        if grouped.get(name):
            rows.append({"name": name, "channels": grouped[name][:12]})
    return rows
...
Write-Host "Reset app\views.py to a clean version." -ForegroundColor Green
